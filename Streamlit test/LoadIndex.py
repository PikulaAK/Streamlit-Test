...
def Index():
    with st.sidebar:
        with st.form("Index_Form"):
            directory = st.file_uploader("Choose index file", type="pkl")
            submitted = st.form_submit_button("Load")
            if submitted:  # Check if a file was uploaded
                file_details = {"FileName":directory.name,"FileType":directory.type}
                st.write(file_details)
                save_uploadedfile(directory)
                st.write("Loading index file...")
                file_path = r"tempDir\\" + directory.name
                index = load_index_from_file(file_path)
                st.write("Loaded index file")
                os.remove(r"tempDir\\"+directory.name)
